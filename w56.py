...
def w56_download_by_id(id, refer, title=None, output_dir='.', merge=True):
	...
	download_urls([url], title, str(ext), total_size=size, n=1, refer=refer, merge=merge)

def w56_download(url, merge=True):
	id = r1(r'http://www.56.com/u\d+/v_(\w+).html', url)
	w56_download_by_id(id, url, merge=merge)
