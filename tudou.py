...
def tudou_download_by_iid(iid, title, refer, merge=True):
    ...
    url = r1(r'<f[^>]*>([^<]*)<', xml)
    # assert 'f4v' in url or 'mp4' in url
    print 'iid = ', iid
    print 'url = ', url
    download_urls([url], title, 'flv', total_size=None, merge=merge)

def __backup_tudou_download_by_iid(iid, title, merge=True):
    url = r'http://v2.tudou.com/v?it=' + iid + r'&st=1,2,3,4,99'
    xml = get_html(url)
    xml = unescape_html(xml)
...
def tudou_download(url, merge=True):
    ...
    tudou_download_by_iid(iid, title, refer=url, merge=merge)
